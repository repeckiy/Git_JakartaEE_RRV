...
<t:template title="Search">
    <h2 class="centered-title">Search</h2>
    <c:choose>
        <c:when test="${empty keyword}">
            <div class="search-form">
                <form action="${pageContext.request.contextPath}/search" method="get">
                    <input type="text" id="keyword" name="keyword" required class="search-input" placeholder="Enter keyword...">
                    <button type="submit" class="button blue">Search</button>
                </form>
            </div>
        </c:when>
        <c:when test="${empty lots}">
            <div class="centered-title" style="font-size: 18px;">No lots found matching your search: <b>${keyword}</b></div>
        </c:when>
        <c:otherwise>
            <div class="centered-title" style="font-size: 18px;">Found ${lots.size()} lot(s) matching: <b>${keyword}</b></div>
            <div class="lots-grid">
